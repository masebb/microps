...
#define ARP_CACHE_TIMEOUT 30 /* secounds */
...
struct arp_ether_ip {
    ...
    uint8_t spa[IP_ADDR_LEN];//ip_addr_tにすると不都合が起きるのでuint8_t
...
arp_dump(const uint8_t *data, size_t len)
{
  ...
  memcpy(&spa, message->spa, sizeof(spa));//諸々の自称(全然わかっていない)でmemcpy
