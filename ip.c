...
ip_route_lookup(ip_addr_t dst)
{
  ...
  for (route = routes; route; route = route->next) {
    //routeがdstのネットワークに含まれているか
    if ((dst & route->netmask) == route->network) { 
      //含まれていても、longest matchで一番サブネットがデカいのを選ぶ
...
struct ip_iface * ip_iface_alloc(const char *unicast, const char *netmask){
  ...
  if (ip_addr_pton(unicast, &iface->unicast) == -1) {
    errorf("ip_addr_pton() fail, unicast=%s", unicast);
    memory_free(iface);
    return NULL;
  } 
  if (ip_addr_pton(netmask, &iface->netmask) == -1) {
    errorf("ip_addr_pton() fail, netmask=%s", netmask);
    memory_free(iface);
    return NULL;
  }
...
int ip_iface_register(struct net_device *dev, struct ip_iface *iface){
  ...
  //インターフェース登録時にルートを登録
  routes = ip_route_add(iface->unicast & iface->netmask, iface->netmask, IP_ADDR_ANY, iface);
...
int ip_protocol_register(uint8_t type, void (*handler)(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)){
  ...
  for (entry = protocols; entry; entry = entry->next) {
    //既に同じtypeのプロトコルがあるかチェック
    if(type == entry->type) {
      errorf("protocol already exist type: %i", entry->type);
      return -1;
    }
  }
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev){
  ...
  if (v != IP_VERSION_IPV4) {
    errorf("not ipv4 protocol v: %u", v);
    return;
  }
  //入力データがヘッダ長より小さい場合
  if (len < hlen ) {
    errorf("data shorter than header length");
    return;
  }
  //チェックサム計算
...
ssize_t ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst) {
  ...
  // 送信元アドレスが指定されていない場合、255.255.255.255への送信はできない(どこのIFから出力するかわからんので)
