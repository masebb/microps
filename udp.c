...
// 疑似ヘッダ
struct pseudo_hdr {
    uint32_t src; //source port
    uint32_t dst; // destination port
    uint8_t zero;
    uint8_t protocol;
    uint16_t len;
};
...
//アプリケーションには生のpcbは触らせたくないので、pcbにidをつけてpcb本体を渡さなくてもいいようにする
static struct udp_pcb *
udp_pcb_get(int id)
{
  struct udp_pcb *pcb;
...
udp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
  ...
  sched_wakeup(&pcb->ctx); //受信キューにエントリーが追加されたので起こす
  mutex_unlock(&mutex);
...
udp_recvfrom(int id, uint8_t *buf, size_t size, struct ip_endpoint *foreign)
{
  ...
  while (1) {
    ...
    // sched_wakeup()もしくはsched_interrupt()が呼ばれるまで寝る
